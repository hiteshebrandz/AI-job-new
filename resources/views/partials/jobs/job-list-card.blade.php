...
<div class="glass-card glass-card-lift p-6 group relative overflow-hidden animate-fade-in">
    ...
    <div class="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-[#7C3AED] to-[#06B6D4] opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>

    <div class="flex items-start justify-between gap-4">
        <!-- Left: Company + Info -->
        <div class="flex gap-4 flex-1 min-w-0">
            <!-- Company Logo -->
            <div class="w-14 h-14 rounded-xl glass-card flex items-center justify-center flex-shrink-0 border border-[#334155]">
            ...
            <div class="min-w-0 flex-1">
                ...
                <p class="text-[14px] text-[#64748B] mb-3">{{ $job->company_name }}</p>

                <!-- Meta tags -->
                <div class="flex flex-wrap gap-3">
                    <div class="flex items-center gap-1.5 text-[#64748B]">
                        <span class="material-symbols-outlined text-[16px] text-[#8B5CF6]" data-icon="location_on">location_on</span>
                        <span class="text-[13px]">{{ $job->displayLocation() }}</span>
                    </div>
                    <div class="flex items-center gap-1.5 text-[#64748B]">
                        <span class="material-symbols-outlined text-[16px] text-[#06B6D4]" data-icon="payments">payments</span>
                        <span class="text-[13px]">{{ $job->displaySalary() }}</span>
                    </div>
                    @if ($savedAt)
                    <div class="flex items-center gap-1.5 text-[#64748B]">
                        <span class="material-symbols-outlined text-[16px] text-[#FBBF24]" data-icon="bookmark">bookmark</span>
                        <span class="text-[13px]">Saved {{ $savedAt->diffForHumans() }}</span>
                    </div>
                    @endif
                    @if ($application)
                    <div class="flex items-center gap-1.5 text-[#64748B]">
                        <span class="material-symbols-outlined text-[16px] text-[#34D399]" data-icon="schedule">schedule</span>
        ...
        <div class="flex flex-col items-end gap-3 flex-shrink-0">
            ...
            <div class="relative w-[68px] h-[68px] flex items-center justify-center">
                <svg class="w-full h-full score-ring -rotate-90" viewBox="0 0 68 68">
                    <circle cx="34" cy="34" r="28" fill="transparent" stroke="#334155" stroke-width="5"/>
                ...
                <div class="absolute inset-0 flex flex-col items-center justify-center">
                    <span class="text-[15px] font-extrabold text-[#C4B5FD]">{{ $matchScore }}%</span>
            ...
            <div class="flex flex-col gap-2">
                ...
                @if ($hasApplied)
                <span class="px-4 py-2 rounded-xl bg-[#263248] text-[#64748B] text-[13px] font-medium text-center border border-[#334155]">Applied</span>
                @else
