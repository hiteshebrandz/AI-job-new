...
@php
    $other = $conversation->candidate;
    $c = $other?->candidate;
    $name = $c?->full_name ?? $other?->name;
@endphp
<div class="pt-[80px] flex flex-col h-[calc(100vh-80px)] max-w-4xl mx-auto px-4 lg:px-8">
    <div class="mb-4 flex items-center gap-3">
        <a href="{{ route('hr.messages.index') }}" class="text-on-surface-variant hover:text-secondary">
            <span class="material-symbols-outlined">arrow_back</span>
        </a>
        <div>
            <h2 class="text-lg font-bold text-on-surface">{{ $name }}</h2>
            @if ($conversation->jobDescription)
                <p class="text-xs text-on-surface-variant">{{ $conversation->jobDescription->title }}</p>
            @endif
        </div>
    </div>

    <div id="chat-messages" class="flex-1 overflow-y-auto glass-card rounded-2xl p-4 space-y-3 mb-4"
         data-list-url="{{ route('hr.messages.list', $conversation) }}"
         data-send-url="{{ route('hr.messages.store', $conversation) }}"
         data-read-url="{{ route('hr.messages.read', $conversation) }}"
    ...
    <form id="chat-form" class="flex gap-2 pb-6">
        ...
        <input type="text" id="chat-input" name="body" placeholder="Type a message…" autocomplete="off"
            class="flex-1 px-4 py-3 rounded-xl border border-outline-variant bg-background text-sm text-on-surface focus:border-secondary">
...
<script src="{{ asset('js/hr-messages.js') }}"></script>
