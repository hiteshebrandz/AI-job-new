            ...
            <div class="text-center">
                <div class="relative w-24 h-24 mx-auto mb-2">
                    <svg class="w-full h-full -rotate-90" viewBox="0 0 96 96">
                        <circle cx="48" cy="48" r="40" fill="transparent" stroke="var(--border-default)" stroke-width="6"/>
                    ...
                    <div class="absolute inset-0 flex flex-col items-center justify-center">
                        <span class="text-[22px] font-extrabold text-secondary" id="score-counter">80</span>
                        <span class="text-[9px] text-on-surface-variant uppercase tracking-wide">Score</span>
                    </div>
                </div>
                <p class="text-[11px] text-on-surface-variant uppercase tracking-wide">AI Profile Score</p>
            </div>
        </div>
    </div>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8 animate-fade-in-delay-1">
    <div class="glass-card stat-card p-5">
        ...
        <p class="text-[28px] font-extrabold text-on-surface mb-1" data-counter="{{ $matchScore }}">0</p>
        <p class="text-[12px] text-on-surface-variant">AI Resume Score</p>
    </div>
    ...
    <div class="glass-card stat-card p-5">
        ...
        <p class="text-[28px] font-extrabold text-on-surface mb-1" data-counter="{{ $appliedCount }}">0</p>
        <p class="text-[12px] text-on-surface-variant">Applications</p>
    </div>

    <div class="glass-card stat-card p-5">
        <div class="flex items-center justify-between mb-3">
            <div class="w-10 h-10 rounded-xl bg-[var(--badge-success-bg)] flex items-center justify-center">
                <span class="material-symbols-outlined text-[var(--badge-success-text)] text-[20px]">bookmark</span>
            </div>
        </div>
        <p class="text-[28px] font-extrabold text-on-surface mb-1" data-counter="{{ $savedCount }}">0</p>
        <p class="text-[12px] text-on-surface-variant">Saved Jobs</p>
    </div>

    <div class="glass-card stat-card p-5">
        <div class="flex items-center justify-between mb-3">
            <div class="w-10 h-10 rounded-xl bg-[var(--badge-warning-bg)] flex items-center justify-center">
                <span class="material-symbols-outlined text-[var(--badge-warning-text)] text-[20px]">notifications</span>
            </div>
            @if($unreadNotifs > 0)
                <span class="badge-ai text-[10px]">{{ $unreadNotifs }} new</span>
            @endif
        </div>
        <p class="text-[28px] font-extrabold text-on-surface mb-1" data-counter="{{ $activeApplications }}">0</p>
        <p class="text-[12px] text-on-surface-variant">Active Applications</p>
    </div>
</div>
...
<div class="grid md:grid-cols-2 gap-5 mb-8 animate-fade-in-delay-2">
    <a href="{{ route('user.resume.upload') }}" class="glass-card glass-card-lift p-7 group relative overflow-hidden">
        ...
        <h2 class="text-[18px] font-bold text-on-surface mb-2">Upload Resume</h2>
        <p class="text-[14px] text-on-surface-variant">Parse and optimize your resume with our AI engine for maximum match accuracy.</p>
    ...
    <a href="{{ route('user.jobs.recommendations') }}" class="glass-card glass-card-lift p-7 group relative overflow-hidden">
        ...
        <h2 class="text-[18px] font-bold text-on-surface mb-2">AI Job Matches</h2>
        <p class="text-[14px] text-on-surface-variant">Browse AI-curated opportunities that align with your skills and experience.</p>
        <div class="flex items-center gap-2 mt-5 text-secondary text-[13px] font-semibold">
...
<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 animate-fade-in-delay-3">
    <a href="{{ route('user.resume.analytics') }}" class="glass-card p-4 flex flex-col items-center text-center gap-2 hover:border-secondary/40 transition-all group">
        <span class="material-symbols-outlined text-secondary text-[26px] group-hover:scale-110 transition-transform">analytics</span>
        <span class="text-[13px] font-medium text-on-surface-variant">Analytics</span>
    </a>
    <a href="{{ route('user.saved-jobs') }}" class="glass-card p-4 flex flex-col items-center text-center gap-2 hover:border-secondary/40 transition-all group">
        <span class="material-symbols-outlined text-[var(--badge-warning-text)] text-[26px] group-hover:scale-110 transition-transform">bookmark</span>
        <span class="text-[13px] font-medium text-on-surface-variant">Saved Jobs</span>
    </a>
    <a href="{{ route('user.applied-jobs') }}" class="glass-card p-4 flex flex-col items-center text-center gap-2 hover:border-secondary/40 transition-all group">
        <span class="material-symbols-outlined text-[var(--badge-success-text)] text-[26px] group-hover:scale-110 transition-transform">assignment_turned_in</span>
        <span class="text-[13px] font-medium text-on-surface-variant">Applications</span>
    </a>
    <a href="{{ route('user.settings.notifications') }}" class="glass-card p-4 flex flex-col items-center text-center gap-2 hover:border-secondary/40 transition-all group">
        <span class="material-symbols-outlined text-on-surface-variant text-[26px] group-hover:scale-110 transition-transform">settings</span>
        <span class="text-[13px] font-medium text-on-surface-variant">Settings</span>
    </a>
</div>
